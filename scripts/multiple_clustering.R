datainput_multiple_clustering <- function(index_multiple_clustering_input, index_multiple_sample_normalization_method, index_m_clustering1, index_m_clustering2, index_m_clustering3, index_m_clustering4, index_m_clustering5, index_m_clustering6, index_m_clustering7, index_m_clustering8, index_m_clustering9, index_m_clustering10, index_m_clustering11, index_m_clustering12, index_m_clustering13){
  ...
  else if (index_m_clustering13 == "HarmonyIntegration")
  {
    if (index_multiple_sample_normalization_method == "LogNormalize"){
      multiple_sample_clustering<- IntegrateLayers(object = index_multiple_clustering_input, method = HarmonyIntegration, normalization.method = "LogNormalize", orig.reduction = "pca", new.reduction = "harmony", verbose = FALSE)
    }
    ...
    { 
      ...
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'harmony_clusters', raster=FALSE, label = index_m_clustering10)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'condition', raster=FALSE, label = index_m_clustering10)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering10)
    }
    else if (index_m_clustering6 == "tsne")
    {
      multiple_sample_clustering<- RunTSNE(multiple_sample_clustering, dims = 1:index_m_clustering11, reduction = "harmony", reduction.name = "tsne")
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'harmony_clusters', raster=FALSE, label = index_m_clustering12)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'condition', raster=FALSE, label = index_m_clustering12)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering12)
    }
    #cell_couts_in_custer
    multiple_sample_clustering_cell_couts_in_custer <- table(multiple_sample_clustering@meta.data$harmony_clusters) %>% as.data.table
    colnames(multiple_sample_clustering_cell_couts_in_custer) <- c("Clusters", "Counts")
    ...
    #cell_couts_in_custer_for_each_condition
    multiple_sample_clustering_cell_couts_in_condition <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_condition <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("condition", "harmony_clusters")] %>% dcast(., condition ~ harmony_clusters, value.var = "N"))) %>%  rownames_to_column(var = "condition") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    ...
    #cell_couts_in_custer_for_each_samples
    multiple_sample_clustering_cell_couts_in_samples <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("orig.ident", "harmony_clusters")] %>% dcast(., orig.ident ~ harmony_clusters, value.var = "N"))) %>%  rownames_to_column(var = "Clusters") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    colnames(multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples)[1] <- "Clusters"
  ...
  else if (index_m_clustering13 == "CCAIntegration")
  {
    if (index_multiple_sample_normalization_method == "LogNormalize"){
      multiple_sample_clustering<- IntegrateLayers(object = index_multiple_clustering_input, method = CCAIntegration, normalization.method = "LogNormalize", orig.reduction = "pca", new.reduction = "cca", verbose = FALSE)
    }
    else if (index_multiple_sample_normalization_method == "SCTransform"){
    ...
    { 
      ...
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'cca_clusters', raster=FALSE, label = index_m_clustering10)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'condition', raster=FALSE, label = index_m_clustering10)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering10)
    }
    else if (index_m_clustering6 == "tsne")
    {
      multiple_sample_clustering<- RunTSNE(multiple_sample_clustering, dims = 1:index_m_clustering11, reduction = "cca", reduction.name = "tsne")
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'cca_clusters', raster=FALSE, label = index_m_clustering12)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'condition', raster=FALSE, label = index_m_clustering12)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering12)
    }
    #cell_couts_in_custer
    multiple_sample_clustering_cell_couts_in_custer <- table(multiple_sample_clustering@meta.data$cca_clusters) %>% as.data.table
    colnames(multiple_sample_clustering_cell_couts_in_custer) <- c("Clusters", "Counts")
    ...
    #cell_couts_in_custer_for_each_condition
    multiple_sample_clustering_cell_couts_in_condition <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_condition <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("condition", "cca_clusters")] %>% dcast(., condition ~ cca_clusters, value.var = "N"))) %>%  rownames_to_column(var = "condition") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    ...
    #cell_couts_in_custer_for_each_samples
    multiple_sample_clustering_cell_couts_in_samples <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("orig.ident", "cca_clusters")] %>% dcast(., orig.ident ~ cca_clusters, value.var = "N"))) %>%  rownames_to_column(var = "Clusters") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    colnames(multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples)[1] <- "Clusters"
  ...
  else if (index_m_clustering13 == "RPCAIntegration")
  {
    ...
    { 
      ...
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'rpca_clusters', raster=FALSE, label = index_m_clustering10)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'condition', raster=FALSE, label = index_m_clustering10)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering10)
    }
    else if (index_m_clustering6 == "tsne")
    {
      multiple_sample_clustering<- RunTSNE(multiple_sample_clustering, dims = 1:index_m_clustering11, reduction = "rpca", reduction.name = "tsne")
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'rpca_clusters', raster=FALSE, label = index_m_clustering12)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'condition', raster=FALSE, label = index_m_clustering12)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering12)
    }
    #cell_couts_in_custer
    multiple_sample_clustering_cell_couts_in_custer <- table(multiple_sample_clustering@meta.data$rpca_clusters) %>% as.data.table
    colnames(multiple_sample_clustering_cell_couts_in_custer) <- c("Clusters", "Counts")
    ...
    #cell_couts_in_custer_for_each_samples
    multiple_sample_clustering_cell_couts_in_samples <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("orig.ident", "rpca_clusters")] %>% dcast(., orig.ident ~ rpca_clusters, value.var = "N"))) %>%  rownames_to_column(var = "Clusters") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    colnames(multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples)[1] <- "Clusters"
  ...
  else if (index_m_clustering13 == "JointPCAIntegration")
  {
    if (index_multiple_sample_normalization_method == "LogNormalize"){
      multiple_sample_clustering<- IntegrateLayers(object = index_multiple_clustering_input, method = JointPCAIntegration, normalization.method = "LogNormalize", orig.reduction = "pca", new.reduction = "jointpca", verbose = FALSE)
    }
    else if (index_multiple_sample_normalization_method == "SCTransform"){
    ...
    { 
      ...
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'jointpca_clusters', raster=FALSE, label = index_m_clustering10)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'condition', raster=FALSE, label = index_m_clustering10)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'umap', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering10)
    }
    else if (index_m_clustering6 == "tsne")
    {
      multiple_sample_clustering<- RunTSNE(multiple_sample_clustering, dims = 1:index_m_clustering11, reduction = "jointpca", reduction.name = "tsne")
      plots16 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'jointpca_clusters', raster=FALSE, label = index_m_clustering12)
      plots17 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'condition', raster=FALSE, label = index_m_clustering12)
      plots18 <-DimPlot(multiple_sample_clustering, reduction = 'tsne', group.by = 'orig.ident', raster=FALSE, label = index_m_clustering12)
    }
    #cell_couts_in_custer
    multiple_sample_clustering_cell_couts_in_custer <- table(multiple_sample_clustering@meta.data$jointpca_clusters) %>% as.data.table
    colnames(multiple_sample_clustering_cell_couts_in_custer) <- c("Clusters", "Counts")
    ...
    #cell_couts_in_custer_for_each_condition
    multiple_sample_clustering_cell_couts_in_condition <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_condition <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("condition", "jointpca_clusters")] %>% dcast(., condition ~ jointpca_clusters, value.var = "N"))) %>%  rownames_to_column(var = "condition") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    ...
    #cell_couts_in_custer_for_each_samples
    multiple_sample_clustering_cell_couts_in_samples <- multiple_sample_clustering@meta.data %>% as.data.table
    multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples <- data.frame(t(multiple_sample_clustering_cell_couts_in_condition[, .N, by = c("orig.ident", "jointpca_clusters")] %>% dcast(., orig.ident ~ jointpca_clusters, value.var = "N"))) %>%  rownames_to_column(var = "Clusters") %>% `colnames<-`(.[1, ]) %>%  .[-1, ]
    colnames(multiple_sample_clustering_total_cell_couts_in_custer_for_each_samples)[1] <- "Clusters"
